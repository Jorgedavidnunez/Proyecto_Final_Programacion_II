...
public class Producto {
    ...
    public int modificar_pVenta(int id, double nuevo_costo,double nuevo_precio) {
        String sql = "update productos set precio_costo=?,Precio_venta=? where idProducto=?;";

        try {
            cn.abrirCon();
            con = cn.conexiondb;
            ps = con.prepareStatement(sql);
            ps.setDouble(1, nuevo_costo);
            ps.setDouble(2, nuevo_precio);
            ps.setInt(3, id);
            ps.executeUpdate();
        } catch (SQLException ex) {
        }
