 ...
 */
public class ser_compras extends HttpServlet {

    int item, item2, cod, id_tabla, table, buscar1, buscar2;
    String descripcion;
    double precio, total;
    int cantidad;
    double subtotal;

    Detalle_compra v = new Detalle_compra();
    List<Detalle_compra> lista = new ArrayList<>();
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet ser_compras</title>");
            out.println("</head>");
            out.println("<body>");
            ...
            if (menu.equals("Compras")) {
                ...
                switch (accion) {
                    ...
                    case "editar":
                        ...
                        buscar1 = Integer.parseInt(request.getParameter("id"));
                        buscar2 = Integer.parseInt(request.getParameter("id33"));
                        Producto2 hr = bh.Listaid(buscar1);
                    ...
                    case "Modificar":
                        ...
                        Detalle_compra vv = new Detalle_compra();
                    ...
                    case "Agregar_compra":
                        for (int e = 0; e < lista.size(); e++) {
                            ...
                            double Precioporc = lista.get(e).getPrecio();//Obtenemos El precio venta
                            pe = pe.Listaid(idproducto44);
                            int sac = pe.getExistencia() + cantidad44;
                            double porcentaje = (Precioporc / 100) * 25 + Precioporc;//Agregamos 25% mas al precio venta
                            pe.modificar_existencia(idproducto44, sac);
                            pe.modificar_pVenta(idproducto44,Precioporc, porcentaje);//Modificamos el Precio venta con la nueva cantidad
                        }

                        compra = new Compras(0, Integer.valueOf(request.getParameter("drop_orden")), Integer.valueOf(request.getParameter("drop_prov")), request.getParameter("txt_fecha"), request.getParameter("fecha_ingreso"));
                        compra.agregar();

                        int idv = Integer.parseInt(compra.Idcompras());
                ...
                request.getRequestDispatcher("index_compras.jsp").forward(request, response);
            }
